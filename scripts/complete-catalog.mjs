#!/usr/bin/env node
// Names the catalog products the supplier export left without a name (see
// scripts/lib/complete-products.mjs): unlabelled colourways get their model's name and a
// colour from their SKU, whole references get the name from
// src/data/product-overrides.<supplier>.json. Patches the committed catalog JSON without
...
for (const supplier of SUPPLIERS) {
	...
	if (report.named > 0 || report.coloured > 0 || report.merged > 0) writeCatalog(catalogPath, products);

	const stillBlank = products.filter((product) => !product.name.trim()).length;
	console.log(
		`${supplier}: ${report.named} product(s) named, ${report.coloured} coloured, ${report.merged} merged into an existing colourway, ${stillBlank} still without a name`,
	);
